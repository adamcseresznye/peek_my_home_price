...
from src.model_training import preprocessing_transformers
...
def create_tuned_pipeline(
    ...
    model_pipeline = pipeline.make_pipeline(
        ...
        catboost.CatBoostRegressor(
            iterations=ModelConfiguration.CATBOOST_ITERATIONS,
            eval_fraction=ModelConfiguration.CATBOOST_EVAL_FRACTION,
            early_stopping_rounds=ModelConfiguration.CATBOOST_EARLY_STOPPING_ROUNDS,
            silent=True,
            use_best_model=True,
            random_seed=ModelConfiguration.RANDOM_SEED,
        ),
    )
    ...
    grid = model_selection.RandomizedSearchCV(
        ...
        n_iter=ModelConfiguration.RANDCV_ITERATIONS,
        n_jobs=-1,
    )
...
def evaluate_model(
    ...
    Returns:
    - pd.DataFrame: A dataframe containing the day of retrieval, the average validation score, and the average test score.
    """
    AVG_val_score = -np.mean(
        model_selection.cross_val_score(
            ...
            y=y_train,
            scoring="neg_root_mean_squared_error",
            cv=ModelConfiguration.CROSSVAL_FOLDS,
        )
    )
